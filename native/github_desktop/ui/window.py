...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _build_changes(self) -> Gtk.Widget:
        ...
        self._commit_status_live = Gtk.Label(xalign=0)
        self._commit_status_live.set_visible(False)
        try:
            self._commit_status_live.update_property(
                [Gtk.AccessibleProperty.LIVE, Gtk.AccessibleProperty.ATOMIC],
                [Gtk.AccessibleLive.POLITE, True],
            )
        except Exception:
            try:
                self._commit_status_live.update_property(
                ...
                pass
        self._commit_form.append(self._commit_status_live)
        self._is_committing_status_message = ""
        self._commit_status_sha = None
        self._was_committing = False
    ...
    def _refresh_repo(self) -> None:
        ...
        self._update_committing_status_message(state)
    ...
    def _update_commit_warnings(self) -> None:
        if not hasattr(self, "_length_hint"):
            return
        ...
        def apply_length_hint(show_rule: bool = False) -> None:
            show = bool(
                summary_length_hint(text, self.store.settings.show_commit_length_warning)
                and not show_rule
            )
            self._length_hint.set_active_hint(show)
            trailing = bool(show or show_rule)
            if hasattr(self, "_summary_row"):
                if trailing:
                    self._summary_row.add_css_class("with-trailing-icon")
                else:
                    self._summary_row.remove_css_class("with-trailing-icon")
        ...
        apply_length_hint(show_rule_hint)
    ...
    def _apply_commit_busy(self, state=None) -> None:
        ...
        if hasattr(self, "_copilot_hint"):
            if generating:
                ...
                self._copilot_hint.set_visible(generated)
        self._announce_generating_commit_details(generating)
        self._update_committing_status_message(state)

    def _update_committing_status_message(self, state=None) -> None:
        """Desktop `isCommittingStatusMessage` / `getButtonTitle` live region."""
        if not hasattr(self, "_commit_status_live"):
            return
        repo = self.store.selected_repository
        state = state or (self.store.state_for(repo) if repo else None)
        local = None
        if state is not None:
            local_shas = set(state.local_commit_shas or [])
            local = next((item for item in state.commits if item.sha in local_shas), None)
        sha = local.sha if local is not None else None
        prev_sha = getattr(self, "_commit_status_sha", None)
        was = getattr(self, "_was_committing", False)
        committing = bool(state and state.is_committing)
        amending = bool(state and state.commit_to_amend is not None)
        branch = state.status.current_branch if state is not None and state.status else None
        if prev_sha is None and not committing:
            self._commit_status_sha = sha
            self._was_committing = committing
            return
        message = getattr(self, "_is_committing_status_message", "")
        if committing and not was:
            message = get_button_title(amending=amending, committing=True, branch=branch)
        elif local is not None and sha != prev_sha:
            message = committing_just_now_message(local.summary, local.short_sha)
        if message:
            self._commit_status_live.set_text(message)
            self._is_committing_status_message = message
        self._commit_status_sha = sha
        self._was_committing = committing

    isCommittingStatusMessage = _update_committing_status_message
    ...
    def _announce_generating_commit_details(self, generating: bool) -> None:
        """Desktop Copilot button AriaLiveContainer `generatingCommitDetailsMessage`."""
        message = generating_commit_details_aria_live(generating)
        target = getattr(self, "_generate_btn", None) or getattr(self, "_copilot_hint", None)
        if target is None:
            return
        prev = getattr(target, "_aria_live_message", object())
        target._aria_live_message = message  # type: ignore[attr-defined]
        if prev == message or not message:
            return
        try:
            target.announce(message, Gtk.AccessibleAnnouncementPriority.MEDIUM)
        except Exception:
            pass
    ...
    def _on_commit_warning_link(self, _label: Gtk.Label, uri: str) -> bool:
        return handle_commit_warning_uri(self.store, uri)
